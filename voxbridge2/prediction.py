...
#
# ✅ Supports 1-hand + 2-hand gestures
# ✅ Uses 126 features (63 hand1 + 63 hand2)
# ✅ No Right/Left classification
# ✅ Stable hand ordering by screen position
# ✅ Finger mode + model mode
# ✅ Firebase writes kept
# ✅ Sends mode + gesture_id
# ✅ Phrase sync kept
# ✅ Phrase combiner kept
# ==============================
...
FEATURES = int(MODEL_INPUT_SHAPE[2])
...
# =====================================================
# ✅ LABELS (9 gestures)
# Change these to match your 9 gesture folders exactly
# =====================================================
gesture_labels = {
    0: "I am",
    1: "Hungry",
...
def extract_features_two_hand(results):
    ...
    detected_hands.sort(key=lambda x: x[0])  # left to right on screen
...
def predict_tflite(seq_input):
    ...
    if seq_input.shape[1] != expected_shape[1] or seq_input.shape[2] != expected_shape[2]:
        print(
            f"❌ Input mismatch: got {seq_input.shape}, "
            f"expected (_, {expected_shape[1]}, {expected_shape[2]})"
        )
        return None
    ...
    with mp_hands.Hands(
        ...
        while not stop_flag:
            try:
                ...
                if results.multi_hand_landmarks:
                    _, detected_hands = extract_features_two_hand(results)
                    ...
                    if len(detected_hands) > 0:
                        finger_hand = detected_hands[0][2]
                ...
                if current_mode == MODE_FINGER:
                    ...
                    if finger_hand is not None:
                        finger_now = count_fingers(finger_hand)
                    else:
                ...
                # =============================================
                    ...
                    if len(detected_hands) > 0:
                        feats, _ = extract_features_two_hand(results)
                    ...
                    if frame_counter % PRED_EVERY_N_FRAMES == 0 and len(sequence) == SEQ_LENGTH:
                        ...
                        if res is not None:
                            ...
                            most_common = Counter(pred_history).most_common(1) if len(pred_history) > 0 else []
                ...
                cv2.putText(display, f"Hands: {len(detected_hands)}",
                            (15, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                            (0, 255, 255), 2)
